const CONFIRM_DELETE_USER_TITLE = '¿Realmente deseas eliminiar a esta material?';
const CONFIRM_DELETE_USER_BODY = `
   Una vez eliminada no podrás recuperarla y deberás crear una nueva. 
   Todas la información asociada a esta material (productos, cotizaciones, etc.) seguirá existiendo.
`;
...
const NEW_material_MODAL = 'new-material-modal';
window.show_new_material_modal = (element)=>{
   $(`#${NEW_material_MODAL}`).modal();
}
